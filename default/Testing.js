        Game.spawns['Spawn1'].spawnCreep([CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE])
        let joe = Game.creeps['Joe']
        if (joe) {
            joe.moveTo(Game.flags.fill)
            if (joe.memory.filled) {
                console.log(joe.transfer(Game.getObjectById('5ad5d89e1db6bf2fc4665134'), RESOURCE_CATALYZED_ZYNTHIUM_ACID))
            } else {
                joe.withdraw(joe.room.terminal, RESOURCE_CATALYZED_ZYNTHIUM_ACID)
                joe.memory.filled = true
            }

        }
        let builder2 = Game.creeps['Builder2']
        if (builder2) {
            //builder2.moveTo(Game.flags['portal'])
        }
        if (builder2) {

            if (builder2.carry.energy === 0) {
                builder2.memory.collecting = true
            } else if (builder2.carry.energy === builder2.carryCapacity) {
                builder2.memory.collecting = false
            }
            if (builder2.memory.collecting) {
                let outcome = builder2.harvest(Game.getObjectById('5982ff42b097071b4adc2524'))
                if (outcome === ERR_NOT_IN_RANGE) {
                    builder2.moveTo(Game.getObjectById('5982ff42b097071b4adc2524'))
                }
            } else {
                let outcome = builder2.build(Game.getObjectById('5b0f132a9421450410f03050'))
                if (outcome === ERR_NOT_IN_RANGE) {
                    builder2.moveTo(Game.getObjectById('5b0f132a9421450410f03050'), {
                        range: 3
                    })
                }
            }
        }
        let builder2 = Game.creeps['ClaimBuilder2']

        if (builder2) {
            if (builder2.pos.roomName === Game.flags['claim']) {
                builder2.moveTo(Game.flags['claim'])
            } else {
                if (builder2.carry.energy === 0) {
                    builder2.memory.collecting = true
                } else if (builder2.carry.energy === builder2.carryCapacity) {
                ...
                if (builder2.memory.collecting) {
                    let outcome = builder2.harvest(Game.getObjectById('59f1a43582100e1594f3ca47'))
                    if (outcome === ERR_NOT_IN_RANGE) {
                        builder2.moveTo(Game.getObjectById('59f1a43582100e1594f3ca47'))
                    }
                } else {
                    let outcome = builder2.upgradeController(Game.getObjectById('59f1a43582100e1594f3ca48'))
                    if (outcome === ERR_NOT_IN_RANGE) {
                        builder2.moveTo(Game.getObjectById('59f1a43582100e1594f3ca48'), {
                            range: 3
                        })
                    }
                }
