class OffensiveModule {
    ...
    wreck(wrecker) {
        ...
        let target = wrecker.pos.findClosestByRange(FIND_STRUCTURES, {
            filter: structure => structure.structureType === STRUCTURE_EXTENSION
        }) //Game.getObjectById('5ad54f0ec0037f40373c6b59')//Game.flags['wreck'].pos.findClosestByRange(FIND_STRUCTURES,{filter:structure => structure.structureType === STRUCTURE_EXTENSION})
        if (target) {
    ...
    rangedSwarmAttack() {
        ...
        if (Memory.isAttacking) {
            ...
            rangedDestroyers.forEach(creep => {
                ...
                creep.moveTo(Game.flags.rangedSwarmAttack, {
                    reusePath: 0,
                    range: 0
                })
        ...
        rangedDestroyers.forEach(creep => {
            ...
            while (moveResult === ERR_INVALID_TARGET) {
                ...
                moveResult = creep.moveTo(new RoomPosition(x, y, roomName), {
                    reusePath: 1
                })
                counter++
            }
